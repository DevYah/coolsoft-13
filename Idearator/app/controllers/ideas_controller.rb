class IdeasController < ApplicationController
    ...
    respond_to do |format|
      if @idea.update_attributes(params[:idea])
        if current_user.provider == 'twitter' && current_user.facebook_share
          current_user.twitter.update("I've updated my idea on #Idearator ! available on: " + root_url + "ideas/" + @idea.id.to_s) rescue Twitter::Error
        end
  ...
  def create
    ...
    respond_to do |format|
      if @idea.save
        ...
        if current_user.provider == 'twitter' && current_user.facebook_share
          current_user.twitter.update("I've created a new idea on #Idearator ! available on: " + root_url + "ideas/" + @idea.id.to_s) rescue Twitter::Error
        end
  ...
  def unarchive
    ...
    if current_user.type == 'Admin' || current_user.id == idea.user_id
      ...
      idea.save
      if current_user.provider == 'twitter' && current_user.facebook_share
        current_user.twitter.update("My idea is back to life! =D I've unarchived my idea on #Idearator ! available on: " + root_url + "ideas/" + idea.id.to_s) rescue Twitter::Error
      end
    else
      respond_to do |format|
        format.html { redirect_to idea, alert: "Idea isn't archived, you are not allowed to archive it." }
